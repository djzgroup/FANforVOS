...
db_root_dir = '/mnt/gmy/DAVIS2017/DAVIS-2017-trainval-480p/'
save_dir = os.path.join(db_root_dir,'first_frame')
with open(os.path.join(db_root_dir, 'ImageSets/2017/', 'val.txt')) as f:
...
for i in range(len(seqnames)):
    ...
    for i in range(100):
        ...
        cv2.imwrite(os.path.join(dream_dir,'%05d.jpg'%i), im_1)

        # Mask for image 1.
        gtim1 = Image.fromarray(gt_1, 'P')
        gtim1.putpalette(palette)
        gtim1.save(os.path.join(dream_dir,'%05d.png'%i))
